...
def gaussianRBF(x, w, theta):
    ...
    return np.squeeze(
        2 * np.exp(-0.5 * np.power(np.tensordot(w, x, axes=(len(w.shape) - 1, 0)) - theta[..., np.newaxis], 2)) - 1)
...
perceptron(gaussianRBF, derrGaussianW, derrGaussianTheta, 10, use_line_search=True, plot_title='Gaussian Activation')
